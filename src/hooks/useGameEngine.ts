    ...
    aiTimerRef.current = setTimeout(() => {
      ...
      if (bestCard) {
        ...
        if (cfg.actionCards.sevenORule && card.type === 'number') {
          const result = applySevenORule(newPlayers, card, state.currentPlayerIndex, state.direction)
          for (let i = 0; i < result.length; i++) {
            newPlayers[i] = result[i]
          }
        }
        ...
        if (card.type === 'wild4' && cfg.actionCards.challengeWild4) {
          ...
          if (nextPlayer) {
            ...
            if (willChallenge) {
              const hadMatching = checkWild4Violation(aiPlayer.hand, state.currentColor)
              if (hadMatching) {
                const returnCard = newDiscardPile.pop()!
                const returnHand = [...newPlayers[state.currentPlayerIndex].hand, returnCard]
                const { drawn: penaltyDrawn, drawPile: aiDrawPile, discardPile: aiDiscardPile } =
                  applyDrawToPlayer(state.drawPile, newDiscardPile, 4)
                const finalHand = [...returnHand, ...penaltyDrawn]
                newPlayers[state.currentPlayerIndex] = { ...newPlayers[state.currentPlayerIndex], hand: finalHand }
                useGameStore.setState({
                  players: newPlayers,
                ...
                const { drawn: penaltyDrawn2, drawPile: aiDrawPile2, discardPile: aiDiscardPile2 } =
                  applyDrawToPlayer(state.drawPile, newDiscardPile, 6)
